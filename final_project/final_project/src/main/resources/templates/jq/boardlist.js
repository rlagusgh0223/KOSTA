$(function () {
  ...
  showList('http://localhost:7888/final_project/final/list', 'currentPage=1')
  //showList('http://localhost:9977/backboardboot/board/list', 'currentPage=1')
  // showList('http://192.168.0.16:9997/backboardboot/board/list')
  ...
  $('div.pagegroup').on('click', 'span:not(.disabled)', function () {
    ...
    //if (!$(this).hasClass('disabled')) {
      ...
      if (word == '') {
        // url = '/backboard/boardlist';
        url = 'http://localhost:7888/final_project/final/list';
        data = 'currentPage=' + pageNo;
      } else {
        // url = '/backboard/searchboard';
        // url = 'http://192.168.0.16:9997/backboardboot/board/list';
        url = 'http://localhost:7888/final_project/final/list';
  ...
  $('div.search>div.searchInput>a').click(function () {
    ...
    // let url = 'http://192.168.0.16:9997/backboardboot/board/list';
    let url = 'http://localhost:7888/final_project/final/list';
 ...
 $('div.boardlist').on('click', 'div.board>div.cell>div.summary>div.arrow', function(){
    if($(this).hasClass('down')){
      ...
      $.ajax({
        ...
        url: 'http://localhost:7888/final_project/final/' + boardNo,
        method:'get',
        // data : 'boardNo='+boardNo,//restful로 url입력하니까따로 데이터 줄 필요 없다
        success:function(jsonObj){
          if(jsonObj.status == 1){
            let board = jsonObj.t;
...
//---삭제버튼 클릭 START---
  ...
  $('div.boardlist').on('click', 'div.board div.reply button', function () {
    ...
    $.ajax({
      "url": "http://localhost:7888/final_project/final/reply" + boardParentNo,
      "method": "POST",
      "timeout": 0,
      "headers": {
        "Content-Type": "application/json"
