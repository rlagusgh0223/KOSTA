...
-- 4. 직책(job_title)이 'President'인 사원의 사번, 이름을 출력하시오: (NaturalJOIN 으로 표현)
SELECT employee_id, first_name
FROM employees NATURAL JOIN jobs  -- NATURAL JOIN할때 별칭 사용 안한다
WHERE job_title = 'President';
...
SELECT e.department_id 부서번호,
    ...
    sum(e.salary) 급여총액, ROUND(AVG(e.salary), 1) 평균급여
FROM employees e
GROUP BY department_id
ORDER BY SUM(e.salary);
...
-- Davies는 제외하고 출력하려면
-- WHERE ~ AND last_name <> 'Davies'



-- 11.성이 'Davies'인 사원과 같은 부서에 근무하면서 'Davies'부서평균급여보다
-- 많은 급여를 받는 사원의 사번, 성, 이름, 급여를 출력하시오(SUBQUERY)
-- 상호연관 서브쿼리
